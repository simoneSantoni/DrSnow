...
\begin{quote}
  \textit{
    ...
    provide more reliable evidence of an association between board gender
diversity
    and corporate risk disclosure.''
  }
\end{quote}

However, Seebeck and Vetter's article does not implement Neyman Potential
Outcome
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}
      ...
      %% axes' end points 
      \draw node[left=3.5cm of p1, align=center] (e) {Extent of the\\intervention};
      \draw node[above=6cm of p0, align=center] (g) {Granularity of the\\intervention};
      \draw node[right=6cm of p0, align=center] (t) {Timescale of the\\intervention};
      %% axes 
      \path[-{Classical TikZ Rightarrow[length=1.5mm]}, draw, shorten <=-4pt, shorten >=1pt]
      (p0) edge node[] {} (e)
      (p0) edge node[] {} (g)
      (p0) edge node[] {} (t);
      %% ticks 
      ...
      \draw node[left=0.25cm of zo] (zol) {One/few units};
      \draw node[left=0.25cm of zp] (zpl) {Population-level};
    ...
    \end{tikzpicture}
    \end{center}
    \caption{A taxonomy of exogenous shocks. \textit{Notes:} the `timescale of the 
    intervention' axis is a continuum; we annotate that axis with reference points
...
The second dimension of our topology --- intervention timescale --- emphasizes
...
\begin{quote}
  \textit{
    ...
    executive net of this pay, relative to the expected incremental value of
the
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}
      ...
      (xc) edge node[] {} (xcl);
      %% reference points 
      \draw node[right=0.75cm of zo, align=center, text=c_1] (b) {B};
      \draw node[right=2.75cm of b, text=c_1] (b1) {B\textsubscript{1}};
      \draw node[above=4.5cm of b, text=c_1] (b2) {B\textsubscript{2}};
      \draw node[right=0.75cm of zp, align=center, text=c_2] (c) {C};
      \draw node[above=4.5cm of c, text=c_2] (c2) {C\textsubscript{2}};
      \draw node[right=2.75cm of c, text=c_2] (c1) {C\textsubscript{1}};
      \draw node[below=0.95cm of p0] (pa) {};
      \draw node[left=0.3cm of pa, align=center, text=c_3] (a) {A};
      \draw node[right=2.75cm of a, text=c_3] (a1) {A\textsubscript{1}};
      \path [
        -{Classical TikZ Rightarrow [length=1.5mm]}, 
        draw=c_3, 
        shorten <=0pt, 
