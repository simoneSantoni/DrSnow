...
\begin{figure}[!htbp]
  ...
  The labels \textit{ER} and \textit{SR}, reported around the arrows 
  connecting events to research questions, denote the empirical relevance 
  and substantive relevance relationships; a naturally occurring 
  event can be associated with one or multiple research questions or be 
  isolated.}
  \label{fig:event_rq_mapping}
\end{figure}

%% substantive relevance
The lack of \textit{substantive relevance} is one of the most common concerns when using naturally occurring events as sources of randomization. For instance, in his critical analysis of Instrumental Variable (IV) applications, \footnote{It is commonly accepted that natural experiments comprise the `Instrumental Variable' design along with the `Standard Natural Experiment' and `Regression Discontinuity Design' \autocite{sieweke_santoni_2020,dunning_2012}.} Deaton \autocite*{deaton_2009} observes that
...
\begin{sidewaysfigure}[!htbp]
  ...
  \begin{tikzpicture}
    ...
    \draw node[below=1cm of p5, align=left] (un) {
      Events known to units
      };
    \draw node[above=1cm of p5, align=left] (kn) {
      Events unknown/\\unknowable to units
      };
...
To attenuate the concern that the units (that is, political leaders' speeches) differ across the sides of the cutoff for reasons other than the exogenous shock,  the authors perform a sensitivity analysis by re-estimating their RDiT mode.

\begin{quote}
  \textit{
    ``[omitted] using only half of the time window on each side of the cutoff,
  ...
  \begin{small}
    ...
    \begin{center}
       ...
       \begin{tabular}{{@{\extracolsep{2pt}} 
         ...
         \multicolumn{1}{c}{$\checkmark$} &
         Terrorist attacks are major exogenous crises that public leaders 
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}
      ...
      \draw node[above=3cm of p0] (yca) {};
      \draw node[above=5cm of p0] (yco) {};
      \draw node[left=0.25cm of yb] (ybl) {};
      \draw node[left=0.25cm of yca] (ycal) {};
      \draw node[left=0.25cm of yco] (ycol) {};
      \draw node[left=0.15cm of ybl] () {Binary};
      \draw node[left=0.15cm of ycal] () {Cardinal};
      \draw node[left=0.15cm of ycol] () {Continuous};
      \path [-, draw, shorten <= -3pt, shorten >=1pt]
      (yb) edge node[] {} (ybl)
      (yca) edge node[] {} (ycal)
      (yco) edge node[] {} (ycol);
      ...
      \draw node[above=3.75cm of b0] (b20) {};
      ...
      \draw node[above=3.75cm of c0] (c20) {};
